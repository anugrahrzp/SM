...
function ServiceCard({ member }) {
  ...
  const handleWhatsAppClick = (e) => {
    e.preventDefault()
    e.stopPropagation()
    const message = `Hi ${member.firstName}, I found your business "${business.name}" on Gang 360 and would like to know more about your services.`
  ...
  return (
    <Link to={`/member/${member.id}`} className="service-card">
      <div className="service-card-header">
        <div className="service-info">
          ...
          <div className="service-meta">
            <span className="service-category">{business.category}</span>
            <span className="service-type-badge">{getTypeLabel(business.type)}</span>
          </div>
        </div>
        <div className="service-owner-avatar">
      ...
      <p className="service-description">{business.description}</p>
      ...
      <div className="service-card-footer">
        <span className="service-owner">by {member.firstName} {member.lastName}</span>
